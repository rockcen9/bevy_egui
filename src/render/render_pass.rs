...
/// Processes postponed paint callback updates for all egui views.
///
/// This must run before [`egui_pass`] in the render schedule.
pub fn update_egui_paint_callbacks(world: &mut World) {
...
/// Egui render pass system.
///
/// This system runs per-camera in the `Core2d` / `Core3d` schedules and performs
/// the actual Egui rendering for the current view.
pub fn egui_pass(
    world: &World,
    ...
    // Query the egui view components.
    let Ok((extracted_view, view_target_comp)) = egui_view_query.get(egui_view_entity) else {
        return;
    };

    let Ok((target, camera)) = egui_view_target_query.get(view_target_comp.0) else {
        return;
    };
    ...
    let mut render_pass = render_ctx.begin_tracked_render_pass(RenderPassDescriptor {
